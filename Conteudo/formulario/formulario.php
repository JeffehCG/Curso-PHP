...
if(count($_POST) > 0) {
    ...
    // isset($_POST['nome'])
    if(!filter_input(INPUT_POST, "nome")) { // Verificando se o campo nome foi preenchido
    ...
    // filter_var - metodo para validação

    if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) { // Validando se email é valido
        $erros['email'] = 'Email inválido';
    }

    if (!filter_var($_POST['site'], FILTER_VALIDATE_URL)) { // Validando se URL é valida
        $erros['site'] = 'Site inválido';
    }

    // Passando configuração para filter_var
    $filhosConfig = [
    ...
    $salarioConfig = [
        'options' => ['decimal' => ','] // Definindo casa decimal como virgula
    ];
